...
public class BvfBleedingEffect extends MobEffect {
    ...
    public static void giveBleedingEffect(LivingEntity target, int step , int maxAmp){
        ...
        MobEffectInstance instance = target.getEffect(BvfMobEffects.BLEEDING.get());
        if(instance == null){
            target.addEffect(new MobEffectInstance(BvfMobEffects.BLEEDING.get(), 1200, 0, false, false, true));
        }else {
            int currentAmp = instance.getAmplifier();
            if (currentAmp + step < maxAmp) {
                instance.update(new MobEffectInstance(BvfMobEffects.BLEEDING.get(), 1200, currentAmp + step, false, false, true));
                previousPositions.clear();
            }else {
                instance.update(new MobEffectInstance(BvfMobEffects.BLEEDING.get(), 1200, maxAmp, false, false, true));
                previousPositions.clear();
            }
        }
    }
    ...
    public static void removeBleedingEffect(LivingEntity target) {
        ...
        int currentAmp = instance.getAmplifier();
        target.removeEffect(BvfMobEffects.BLEEDING.get());

        int duration = instance.getDuration();
        if(currentAmp > 0) {
            target.addEffect(new MobEffectInstance(BvfMobEffects.BLEEDING.get(), duration, currentAmp - 1, false, false, true));
        }
