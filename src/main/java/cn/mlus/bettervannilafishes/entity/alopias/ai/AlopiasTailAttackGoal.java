...
public class AlopiasTailAttackGoal extends MeleeAttackGoal {
    ...
    @Override protected void checkAndPerformAttack(@NotNull LivingEntity target, double distance) {
        if (distance <= getAttackReachSqr(target) && getTicksUntilNextAttack() <= 0) {
            ...
            TickHelper.tickLater(shark.level(), 10, () -> {
                if (target.isAlive() && shark.distanceToSqr(target) <= getAttackReachSqr(target) + 2) {
