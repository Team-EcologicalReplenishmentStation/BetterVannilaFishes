...
public class SquidMoveControl extends BvfWaterAnimalMoveControl {
    ...
    public void tick() {
        if (squid.isAggressive() && squid.isInWater() && this.operation == Operation.MOVE_TO) {
            ...
            // Actual movement: apply velocity towards target (i.e., 'behind' the model's perspective)
            // Minecraft forward direction = (-sin(yaw), cos(yaw)), move towards target using towardTarget angle
